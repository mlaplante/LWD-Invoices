...
export type ReconciliationPayment = {
  ...
  matchedAmount: number | { toString(): string };
...
export function MatchPaymentDialog({
  ...
  const currency =
    currencies?.find((item) => item.isDefault) ?? currencies?.[0];
  ...
  const invoices = trpc.paymentReconciliation.openInvoices.useQuery(
  ...
  const match = trpc.paymentReconciliation.match.useMutation({
    onSuccess: async () => {
      await Promise.all([
        utils.paymentReconciliation.list.invalidate(),
  ...
  function addInvoice(invoice: {
    ...
    if (allocations.some((allocation) => allocation.invoiceId === invoice.id))
      return;
    setAllocations((current) => [
      ...current,
      {
        ...
        amount: prefillAllocation(
          invoice.balance,
          available - current.reduce((sum, item) => sum + item.amount, 0),
        ),
      },
    ]);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            Match payment{payment.payerName ? ` from ${payment.payerName}` : ""}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="invoice-search">Find open invoices</Label>
            <Input
              id="invoice-search"
              value={search}
            ...
            <div className="space-y-2">
              {allocations.map((allocation) => (
                <div
                  ...
                  className="grid grid-cols-[1fr_9rem_auto] items-center gap-2 rounded-lg border p-2"
                >
                  <div>
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">
                      Balance {money(allocation.balance)}
                    </span>
                  </div>
                  <Input
                    aria-label={`Allocation for ${allocation.number}`}
                    type="number"
                    min="0"
                    step="0.01"
                    value={allocation.amount}
                    onChange={(event) =>
                      setAmount(allocation.invoiceId, event.target.value)
                    }
                  />
                  <Button
                    type="button"
                    ...
                    onClick={() =>
                      setAllocations((current) =>
                        current.filter(
                          (item) => item.invoiceId !== allocation.invoiceId,
                        ),
                      )
                    }
                  >
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              allocations.length === 0 || remaining < -0.005 || match.isPending
            }
            onClick={() =>
              match.mutate({
                id: payment.id,
          ...
          >
            {match.isPending ? "Matching…" : "Match payment"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
