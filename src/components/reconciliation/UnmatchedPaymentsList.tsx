...
export function UnmatchedPaymentsList() {
  ...
  const status:
    ...
    | undefined = showHistory
    ? ["UNMATCHED", "PARTIALLY_MATCHED", "IGNORED", "MATCHED"]
    : undefined;
  const payments = trpc.paymentReconciliation.list.useQuery({ status });
  const ignore = trpc.paymentReconciliation.ignore.useMutation({
    onSuccess: async () => {
  ...
  const unignore = trpc.paymentReconciliation.unignore.useMutation({
    onSuccess: async () => {
      await utils.paymentReconciliation.list.invalidate();
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/40 text-left text-xs text-muted-foreground">
              <tr>
                ...
                <th className="p-3">Payer / reference</th>
            ...
            <tbody>
              {payments.data.map((payment) => {
                const remaining =
                  number(payment.amount) - number(payment.matchedAmount);
                return (
                    ...
                    <td className="p-3 text-right space-x-2">
                      ...
                      {payment.status === "IGNORED" && (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => unignore.mutate({ id: payment.id })}
                        >
                          Restore
                        </Button>
                      )}
      ...
      <AddUnmatchedPaymentDialog open={addOpen} onOpenChange={setAddOpen} />
      <MatchPaymentDialog
        payment={matching}
        open={Boolean(matching)}
        onOpenChange={(open) => {
          if (!open) setMatching(null);
        }}
      />
    </>
  );
