...
import { Send, MailCheck, Eye, MousePointerClick, AlertTriangle, Ban, Mail } from "lucide-react";

type Props = { invoiceId: string; hasSent: boolean };
...
export function EmailEngagementPanel({ invoiceId, hasSent }: Props) {
  const { data: events = [], isLoading } = trpc.invoices.getEmailEvents.useQuery({ invoiceId });

  // Nothing to show and the invoice was never emailed — stay out of the way.
      ...
      <div className="rounded-2xl border border-border/50 p-5">
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            ...
            recipient interacts with the email.
          </p>
        ) : (
