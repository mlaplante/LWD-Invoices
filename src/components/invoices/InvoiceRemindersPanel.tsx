...
/**
 * Combined reminder history for an invoice — manual one-click sends and
 * automated sequence sends, newest first. Renders nothing until there's at
 * least one reminder, keeping the invoice page uncluttered.
 */
export function InvoiceRemindersPanel({ invoiceId }: { invoiceId: string }) {
  const { data } = trpc.invoices.reminderHistory.useQuery({ invoiceId });
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {r.kind === "manual" ? (
                  <>
                    ...
                    {r.tone ? ` · ${TONE_LABELS[r.tone] ?? r.tone}` : ""}
                  </>
                ) : (
