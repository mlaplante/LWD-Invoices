...
<form method="post" action="<?php echo $post_url; ?>" id="payment-form" class="client_fields">
        ...
        <?php else: ?>
            <div class="row">
                <?php if ($field['type'] == 'checkbox'): ?>
                    <label class="checkbox-label" for="<?php echo $key; ?>">
                        <input type="checkbox" autocomplete="off" <?php if ($use_field_names): ?>name="client_fields[<?php echo $key; ?>]"<?php endif; ?> value="1" id="<?php echo $key; ?>"/>
                ...
                <?php else: ?>
                    ...
                    <?php elseif ($field['type'] == 'select'): ?>
                        <select id="<?php echo $key; ?>" <?php if ($use_field_names): ?>name="client_fields[<?php echo $key; ?>]"<?php endif; ?>>
    ...
    <div class="row">
        <button type="submit" class="submit-button">
            <span class="not-disabled">
                <?php echo __('partial:proceedtopayment'); ?>
            </span>
            <span class="disabled">
                <?php echo __('update:loadingpleasewait'); ?>
            </span>
        </button>
    </div>
</form>