 ...
 * @version         3.0.0-pre
 */
abstract class REST_Controller extends Pancake_Controller
{
	/**
	...
	*/
	...
	public function __construct()
	{
		...
		$this->_get_args = array_merge($this->_get_args, $_GET, $this->uri->ruri_to_assoc());
	...
	protected function _log_request($authorized = FALSE)
	{
		...
		if (is_array($args)) {
            ...
            if (isset($args['files'])) {
                foreach ($args['files'] as $key => $details) {
                    if (is_array($details) && isset($details["contents"])) {
                        $args['files'][$key]["contents"] = null;
                    }
                }
            }
		}
