...
class Template
{
	...
	public function css($css) {
		...
		if (stristr($trace[0]['file'], FCPATH."/third_party/modules/") !== false) {
			throw new Exception("Calling css()/js() from outside a plugin is not allowed");
		} else {
			$plugin = str_ireplace(FCPATH."third_party/modules/", '', $trace[0]['file']);
			$plugin = array_reset(explode("/", $plugin));
		}

		$this->plugin_css[] = Asset::get_src_from_base("third_party/modules/$plugin/$css");
	}
	...
	public function js($js) {
		...
		$this->plugin_js[] = Asset::get_src_from_base("third_party/modules/$plugin/$js");
	}
	...
	{
		get_instance()->benchmark->mark("template_build_start");

		// Set whatever values are given. These will be available to all view files
		is_array($data) OR $data = (array) $data;

		$data['plugin_js'] = $this->plugin_js;
		$data['plugin_css'] = $this->plugin_css;
		...
		$this->_ci->output->set_header('Cache-Control: private');
		...
		get_instance()->benchmark->mark("template_build_end");
		return $this->_body;
	}
