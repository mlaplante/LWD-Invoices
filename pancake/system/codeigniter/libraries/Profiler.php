...
class CI_Profiler {
	...
	protected function _compile_benchmarks()
	{
		...
		$key = "Total Query Time";
		$val = number_format(array_sum($this->CI->db->query_times), 4);
		$query_time = $val;
		$time_accounted_for = $query_time;
		...
		foreach ($profile as $key => $val)
		{
			...
			if (substr($key, 0, strlen("controller_execution_time")) != "controller_execution_time" && $key != "total_execution_time") {
				$time_accounted_for += $val;
			}
		...
		$key = "Time Unaccounted For";
		$total_time = $profile["total_execution_time"];
		$val = $total_time - $time_accounted_for;
		if ($val < 0) {
			$val += round($query_time, 4);
		}
		$output .= "<tr><td style='padding:5px;width:50%;color:#000;font-weight:bold;background-color:#ddd;'>".$key."&nbsp;&nbsp;</td><td style='padding:5px;width:50%;color:#900;font-weight:normal;background-color:#ddd;'>".$val."</td></tr>\n";
	...
	protected function _compile_queries()
	{
		...
		foreach ($dbs as $db)
		{
			...
			$output .= "\n";
			$output .= '<legend style="color:#0000FF;">&nbsp;&nbsp;'.$this->CI->lang->line('profiler_database').':&nbsp; '.$db->database.'&nbsp;&nbsp;&nbsp;Total Query Time: '.number_format(array_sum($db->query_times), 4).'&nbsp;&nbsp;&nbsp;'.$this->CI->lang->line('profiler_queries').': '.count($db->queries).'&nbsp;&nbsp;&nbsp;IMPROVABLE QUERIES: {{improvable_queries}}&nbsp;&nbsp;'.$show_hide_js.'</legend>';
			$output .= "\n";
			$output .= "\n\n<style>.the-query * {white-space: normal !important;}</style><table style='width:100%;{$hide_queries}' id='ci_profiler_queries_db_{$count}'>\n";
				...
				foreach ($db->queries as $key => $val)
				{
					...
					foreach ($highlight as $bold)
					{
						$val = str_replace($bold, '<strong>'.$bold.'</strong>', $val);
                                        ...
                                        $trace = "";
                                        $frequency_key = uniqid();
                                        
                                        if (stristr($db->traces[$key][0]['file'], "codeigniter/database/DB.php") === false) {

                                            $trace = reset($db->traces[$key]);
                                            $trace_key = array_search($trace, $db->traces[$key]);
                                            if (stristr($trace['file'], "codeigniter/database/DB_active_rec.php") !== false and $trace['function'] == "query") {
                                                unset($db->traces[$key][$trace_key]);
                                            }

                                            $trace = reset($db->traces[$key]);
                                            $trace_key = array_search($trace, $db->traces[$key]);
                                            if (stristr($trace['file'], "codeigniter/database/DB_query_builder.php") !== false and $trace['function'] == "query") {
                                                unset($db->traces[$key][$trace_key]);
                                            }
                                            
                                            $trace = reset($db->traces[$key]);
                                            $trace_key = array_search($trace, $db->traces[$key]);
                                            if (stristr($trace['file'], "codeigniter/database/Pancake_Model.php") !== false) {
                                                unset($db->traces[$key][$trace_key]);
                                            }
                                            
                                            $trace = reset($db->traces[$key]);
                                            $trace_key = array_search($trace, $db->traces[$key]);
                                            if (stristr($trace['file'], "pancake/core/Pancake_Model.php") !== false) {
                                                unset($db->traces[$key][$trace_key]);
                                            }
                                            ...
                                            if (isset($trace['file'])) {
                                                ...
                                                $lines = file($source);
                                                $source = "";
                                                $source .= $lines[$trace['line'] - 3];
                                                $source .= $lines[$trace['line'] - 2];
                                                $source .= $lines[$trace['line'] - 1];
                                                $source .= $lines[$trace['line']];
                                                $source .= $lines[$trace['line'] + 1];

                                                $trace['file'] = str_ireplace(FCPATH, "", $trace['file']).":".$trace["line"];

                                                if (!isset($frequencies[$trace['file']])) {
                                                    $frequencies[$trace['file']] = 0;
                                                }

                                                $frequencies[$trace['file']]++;
                                                $frequency_key = $trace['file'];

                                                if (!isset($lags[$trace['file']])) {
                                                    $lags[$trace['file']] = 0;
                                                }

                                                $lags[$trace['file']] += $db->query_times[$key];
                                                $lag_key = $trace['file'];

                                                $trace = "<strong>".$trace['file']."</strong>\n";
                                                $trace .= $source;
                                                $trace = nl2br($trace);
                                            } else {
												$lag_key = "n/a";
												$trace = "Could not build trace.";
                                            }
                                        } else {
											$lag_key = "n/a";
										}
                                        ...
                                        $time = isset($lags[$lag_key]) ? number_format($lags[$lag_key], 4) : "n/a";
                                                 
                                        $buffer = "<tr><td style='padding:5px; vertical-align: top;width:1%;color:#900;font-weight:normal;background-color:#ddd;'>{{frequency}}</td><td style='padding:5px; vertical-align: top;width:1%;color:#900;font-weight:normal;background-color:#ddd;'>".$time."&nbsp;&nbsp;</td><td style='padding:5px; color:#000;font-weight:normal;background-color:#ddd;' class='the-query'>".$val."$trace</td></tr>\n";
                                        $frequencies_output[$frequency_key] = $buffer;
                                        $lags_output[$lag_key] = $buffer;
				}
                                ...
                                foreach ($lags as $key => $value) {
                                    if ($value < 0.1) {
                                        unset($lags[$key]);
                                    } else {
                                        $lags_output[$key] = str_ireplace("{{frequency}}", $frequencies[$key]."x", $lags_output[$key]);
                                        $valid_outputs[$key] = $lags_output[$key];
                                    }
                                    
                                    unset($lags_output[$key]);
                                }
                                
                                foreach ($lags_output as $key => $value) {
                                    $value = str_ireplace("{{frequency}}", "n/a", $value);
                                ...
                                foreach ($frequencies as $key => $value) {
                                    if ($value < 2) {
                                        ...
                                        $frequencies_output[$key] = str_ireplace("{{frequency}}", $value."x", $frequencies_output[$key]);
                                        $valid_outputs[$key] = $frequencies_output[$key];
                                    }
                                    
                                    unset($frequencies_output[$key]);
                                }
                                
                                foreach ($frequencies_output as $key => $value) {
                                    $value = str_ireplace("{{frequency}}", "n/a", $value);
                                    $valid_outputs[$key] = $value;
                                }
                                
                                $output = str_ireplace("{{improvable_queries}}", (count($frequencies) + count($frequencies_output) + count($lags) + count($lags_output)), $output);
                                $output .= str_ireplace("&nbsp;", " ", implode("", $valid_outputs));
			}
