...
| 7 | **Lint is a ratcheted gate** | `ci.yml` runs `npm run lint:ci` (`eslint --max-warnings <n>`; ESLint stays on 9 because eslint-config-next's vendored eslint-plugin-react crashes on 10 — see `.ncurc.yml`). Errors fail CI outright; warnings fail CI only when they exceed the number committed in `package.json`, so the pre-existing `react-hooks/*` backlog can't silently regrow. | Keep `npm run lint` at zero errors and add no new warnings. When you fix warnings, lower `--max-warnings` in `package.json` in the same PR. | `lwd-validation-and-qa` |
